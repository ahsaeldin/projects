...
Namespace UI
    Friend Class CommonActionForm

#Region "Properties"
        Property TargetTask As Task
        Property IsLoaded As Boolean = False
        Property IsEditMode As Boolean = False
        Property CreatedAction As New StartApplication() ''''''''''''''''''''''''''''''''''''''@@@@@@@@@@@@@@@@@@@@@@@@@@ change it to the desired action
#End Region
...
#Region "Form Events"
        Private Sub ActionForm_Load(sender As System.Object, e As System.EventArgs) Handles MyBase.Load
            ...
            'Note that you can bypass enviroment viriables if you wish.
            ...
            Public Overrides Function Validate(control As System.Windows.Forms.Control, value As Object) As Boolean
                Select Case control.Name
                    Case "RunAnotherTaskComboBoxEdit"
                        If IsNothing(value) And CommonActionForm.OnErrorsRadioGroup.SelectedIndex = 3 Then
                            Return False
                        End If
                End Select
                Return True
