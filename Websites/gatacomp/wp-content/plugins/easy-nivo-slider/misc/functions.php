	...
	<script language="JavaScript" type="text/javascript">
		jQuery(document).ready(function($) {
			$("#slider-<?php echo $parms['ID']; ?>").nivoSlider({
				...
				beforeChange: function(){$("#big-image-comment-content").slideUp();},
				afterChange: function(){					
					$("#big-image-comment-content").html($(".nivo-caption").html());
					$("#big-image-comment-content").slideDown(2000);},
				afterLoad: function(){
					<?php 
						if (is_page('Home') || is_page('الصفحة الرئيسية') ){
							echo('$(".nivo-caption").css("visibility","hidden");');
						}
					?>
					$("#big-image-comment-content").html($(".nivo-caption").html());
					}	
