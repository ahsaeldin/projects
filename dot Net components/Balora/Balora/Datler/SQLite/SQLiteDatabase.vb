...
'created or managed by System.Data.SQLite provider http://system.data.sqlite.org/index.html/doc/trunk/www/index.wiki 
...
#Region "Methods"
        ...
        Public Overrides Function IsTableExists(ByVal dbConn As DbConnection, ByVal tableName As String) As Boolean
            ...
            Try
                tableRows = dbConn.GetSchema("Tables").Select("Table_Name = '" & tableName & "'")
            Catch ex As Exception
                Balora.Alerter.RaiseErrorUp("", ex, True)
                Return False
            End Try
            If tableRows.Length = 0 Then Return False Else Return True
            'Another primitive way to check if table exists...
            'doesTableExist(CType(dbConn, SQLiteConnection), tableName)
            'Public Function doesTableExist(ByVal theDatabase As SQLiteConnection, ByVal tableName As [String]) As [Boolean]
            '    Dim cmd As New SQLiteCommand(theDatabase)
        ...
        Private Function ExecuteQuery(ByVal selectCommand As String, Optional ByVal selectParameters As Dictionary(Of String, Object) = Nothing) As DataTable
            ...
            Try
                Dim queryDataTable As New DataTable
                Using SQLcommand As New SQLiteCommand(dbConnectionValue)
        ...
        Public Shared Function ObjectToBlob(ByVal obj As Object) As Byte()
            If TypeOf obj Is Image Then
                Return Util.ImageToByteArray(CType(obj, Image))
            Else
        ...
        Friend Overridable Function Encrypt(password As String) As Boolean
            Try
                Dim reConnectionResult As Boolean = ReConnect(DatabaseConnection)
                If reConnectionResult = False Then Return Nothing
                DatabaseConnection.ChangePassword(password)
        ...
        Friend Overridable Function Decrypt(password As String) As Boolean
            Try
                DatabaseConnection.SetPassword(password)
                Dim reConnectionResult As Boolean = ReConnect(DatabaseConnection)
                If reConnectionResult = False Then Return Nothing
                DatabaseConnection.ChangePassword("")
                DatabaseConnection.Close()
                Return True
            Catch ex As Exception
                Balora.Alerter.RaiseErrorUp("", ex, True)
...
#Region "IDisposable Support"
        ...
        Protected Overridable Sub Dispose(ByVal disposing As Boolean)
            If Not Me.disposedValue Then
                ...
                If Not dbConnectionValue Is Nothing Then dbConnectionValue.Dispose()
                If Not dataAdapterValue Is Nothing Then dataAdapterValue.Dispose()
                If Not dataSetValue Is Nothing Then dataSetValue.Dispose()
            End If
...
#Region "HowTOUse"
        ...
        Private Sub HowToUse()
            ...
            '      ConnectionFactory = New ConnectionFactory(DbType.SQLite, Dbprovider.System_Data_SQLite) 'Database Type Change: Change: Only one remainings.(3/4)
            '      Try
